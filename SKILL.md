...
| **4. Resize images** | Finds image page-elements whose effective width (`size.width × transform.scaleX`) exceeds `--max-image-width`, then `updatePageElementTransform` with a uniform shrink factor preserving aspect and anchor position. | Defensive sweep — pptx normally sizes images correctly, but some converters scale up beyond the page bounds. Default `720pt` matches Slides' 16:9 page width. |
